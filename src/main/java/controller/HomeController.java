...
//	public String orderProcess(@PathVariable("userid") String userid, @RequestParam("usedPoint") double usedPoint,@RequestParam("payTypeCode") String payTypeCode, ProfileVo profileVo, HttpSession session) {
//		itemList = (List<OrderedItemsVo>) session.getAttribute("itemList");
//		
//		for (int i = 0; i < itemList.size(); i++) {
//			System.out.println(itemList.get(i).getBasketId());
//			OrderedItemsVo  orderVo = itemList.get(i);
//			buyService.updateBakset(orderVo.getBasketId(), orderVo.getDeliveryFee(), usedPoint, payTypeCode);
//			buyService.changeOrderStatus(orderVo.getBasketId(), "OR03");
//		}
...
//	}
	...
	public String orderProcess(@PathVariable("userid") String userid 
							 ...
							 , @RequestParam("cardPeriod") String cardPeriod
		...
		profileVo.setCardCompanyCode(cardCompanyCode);
		profileVo.setCardNumber(cardNumber);
		profileVo.setCardPeriod(cardPeriod);
		profileVo.setCvc(cvc);
		profileVo.setCardPassword(cardPassword);
