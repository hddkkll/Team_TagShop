...
function basketListDelete(userid, productitemId, buyitemsId){
	...
	$.when(deleteBasketItem(curl1), basketList(curl2, userid) ).done(function( result1, resulte2) {
		console.log(result1);      
			...
			$.each(data,function(index, item){
				...
				jsontable +=
					...
					'<div  class="product-image"><img src="'+ctx+'/uploads/'+ item.photoFile + '" class="cart-thumb imgbasket" alt="">' +
						...
						'<p class="size">SIZE: '+ item.options[1].optionCode + '</p>' +
						'<p class="color">Color: '+ item.options[0].optionCode + '</p>'+
